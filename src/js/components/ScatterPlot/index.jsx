...
const ScatterPlot = () => {
  ...
  useEffect(() => {
    const getPastData = async () => {
      const values =
        await d3.csv("/datasets/Past.csv", mapRaw)
      setPastData(values)
    }
    getPastData();
  }, []);
  ...
  useEffect(() => {
    const getFutureData = async () => {
      const values =
        await d3.csv("/datasets/Future.csv", mapRawFuture)
  ...
  const ref = useD3(
    (svg) => {
      if(pastData && futureData) {
        ...
        const yScale = d3.scaleLinear([minY, maxY],[height-margin.bottom, margin.top])
